...
from squid_control.control.core import Configuration
...
class PlateReadingWorker(QObject):
    ...
    def run_single_time_point(self):
        ...
        for column in self.selected_columns:
            ...
            # go through rows
            for row in range(CONFIG.PLATE_READER.NUMBER_OF_ROWS):

                if row_scan_direction == 0:  # reverse scan:
                    row = CONFIG.PLATE_READER.NUMBER_OF_ROWS - 1 - row
                ...
                if (
                    ...
                    and (
                        self.FOV_counter % CONFIG.Acquisition.NUMBER_OF_FOVS_PER_AF == 0
                    )
                ):
...
class PlateReadingController(QObject):
    ...
    def __init__(
        ...
        mm_per_ustep_X = CONFIG.SCREW_PITCH_X_MM / (
            self.plateReaderNavigationController.x_microstepping
            * CONFIG.FULLSTEPS_PER_REV_X
        )
        mm_per_ustep_Y = CONFIG.SCREW_PITCH_Y_MM / (
            self.plateReaderNavigationController.y_microstepping
            * CONFIG.FULLSTEPS_PER_REV_Y
        )
        mm_per_ustep_Z = CONFIG.SCREW_PITCH_Z_MM / (
            self.plateReaderNavigationController.z_microstepping
