...
import squid_control.control.core_PDAF as core_PDAF
...
class OctopiGUI(QMainWindow):
    ...
    def __init__(self, is_simulation=False, *args, **kwargs):
        ...
        self.twoCamerasPDAFCalibrationController = (
            core_PDAF.TwoCamerasPDAFCalibrationController(
                self.camera_1,
                self.camera_2,
                self.navigationController,
                self.liveController_1,
                self.liveController_2,
                self.configurationManager,
            )
        )
        ...
        self.liveControlWidget_1 = widgets.LiveControlWidget(
            self.streamHandler_1, self.liveController_1, self.configurationManager
        )
        ...
        self.liveControlWidget_2 = widgets.LiveControlWidget(
            self.streamHandler_2, self.liveController_2, self.configurationManager
        )

        self.PDAFCalibrationWidget = widgets.MultiPointWidget(
            self.twoCamerasPDAFCalibrationController, self.configurationManager
        )
        ...
        self.twoCamerasPDAFCalibrationController.image_to_display_camera1.connect(
            self.imageDisplayWindow_1.display_image
        )
        self.twoCamerasPDAFCalibrationController.image_to_display_camera2.connect(
            self.imageDisplayWindow_1.display_image
        )
        self.twoCamerasPDAFCalibrationController.signal_current_configuration.connect(
            self.liveControlWidget_1.set_microscope_mode
        )
        self.twoCamerasPDAFCalibrationController.signal_current_configuration.connect(
