import squid_control.control.utils.image_processing as image_processing
import numpy as np
from os.path import realpath, dirname, join

try:
    import torch
    from squid_control.control.DaSiamRPN.code.net import SiamRPNvot

    print(1)
    from squid_control.control.DaSiamRPN.code import vot

    print(2)
    from squid_control.control.DaSiamRPN.code.utils import (
        get_axis_aligned_bbox,
        cxy_wh_2_rect,
    )

    print(3)
    from squid_control.control.DaSiamRPN.code.run_SiamRPN import (
        SiamRPN_init,
        SiamRPN_track,
    )
...
class Tracker_Image(object):
    ...
    def __init__(self):
        ...
        # Image Tracker type
        self.tracker_type = CONFIG.Tracking.DEFAULT_TRACKER
        # Init method for tracker
        self.init_method = CONFIG.Tracking.DEFAULT_INIT_METHOD
        # Create the tracker
        self.create_tracker()
        ...
        if is_first_frame == True or self.trackerActive == False:
            ...
            # tracker initialization - using thresholded image
                ...
                self.bbox = image_processing.scale_square_bbox(
                    self.bbox, CONFIG.Tracking.BBOX_SCALE_FACTOR, square=True
                )
    ...
    def _initialize_tracker(self, image, centroid, bbox):
        ...
        # Initialize Neural Net based Tracker
            ...
            target_pos, target_sz = np.array([centroid[0], centroid[1]]), np.array(
                [bbox[2], bbox[3]]
            )
            if self.is_color == False:
                image = cv2.cvtColor(image, cv2.COLOR_GRAY2BGR)
