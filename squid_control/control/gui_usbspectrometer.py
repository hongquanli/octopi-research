...
import squid_control.control.widgets_usbspectrometer as widgets_usbspectrometer
import squid_control.control.core_usbspectrometer as core_usbspectrometer
import squid_control.control.spectrometer_oceanoptics as spectrometer
...
class OctopiGUI(QMainWindow):
    ...
    def __init__(self, is_simulation=False, *args, **kwargs):
        ...
        self.imageSaver = core.ImageSaver(image_format=CONFIG.Acquisition.IMAGE_FORMAT)
        ...
        if CONFIG.ENABLE_STROBE_OUTPUT:
            self.camera.set_line3_to_exposure_active()

        self.spectrometer.set_callback(
            self.spectrometerStreamHandler.on_new_measurement
        )
        ...
        self.spectrumRecordingWidget = widgets_usbspectrometer.RecordingWidget(
            self.spectrometerStreamHandler, self.spectrumSaver
        )
        if CONFIG.ENABLE_TRACKING:
        ...
        self.recordTabWidget.addTab(
            self.spectrumRecordingWidget, "Recording - Spectrometer"
        )
        self.recordTabWidget.addTab(self.multiPointWidget, "Multipoint Acquisition")
        self.spectrometerControlWidget = (
            widgets_usbspectrometer.SpectrometerControlWidget(
                self.spectrometer, self.spectrometerStreamHandler
            )
        )
        ...
        self.spectrometerStreamHandler.spectrum_to_display.connect(
            self.spectrumDisplay.plot
        )
        self.spectrometerStreamHandler.spectrum_to_write.connect(
            self.spectrumSaver.enqueue
        )
