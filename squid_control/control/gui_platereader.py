...
import squid_control.control.core_platereader as core_platereader
...
class OctopiGUI(QMainWindow):
    ...
    def __init__(self, is_simulation=False, *args, **kwargs):
        ...
        self.configurationManager = core.ConfigurationManager(
            filename=str(Path.home()) + "/configurations_platereader.xml"
        )
        ...
        self.plateReaderNavigationController = core.PlateReaderNavigationController(
        ...
        self.plateReaderAcquisitionWidget = widgets.PlateReaderAcquisitionWidget(
            self.plateReadingController,
            self.configurationManager,
            show_configurations=False,
        )
        self.plateReaderNavigationWidget = widgets.PlateReaderNavigationWidget(
            self.plateReaderNavigationController
        )
        ...
        self.plateReadingController.signal_current_configuration.connect(
            self.liveControlWidget.set_microscope_mode
        )
        self.plateReadingController.image_to_display.connect(
            self.imageDisplayWindow.display_image
        )
        ...
        self.microcontroller.set_callback(
            self.plateReaderNavigationController.update_pos
        )
        self.plateReaderNavigationController.signal_homing_complete.connect(
            self.plateReaderNavigationWidget.slot_homing_complete
        )
        self.plateReaderNavigationController.signal_homing_complete.connect(
            self.plateReaderAcquisitionWidget.slot_homing_complete
        )
        self.plateReaderNavigationController.signal_current_well.connect(
            self.plateReaderNavigationWidget.update_current_location
        )
