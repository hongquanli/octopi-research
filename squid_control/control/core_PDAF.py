...
from squid_control.control.core import *
...
class TwoCamerasPDAFCalibrationController(QObject):
    ...
    def __init__(
        ...
        self.deltaZ_usteps = round(
            (CONFIG.Acquisition.DZ / 1000) * CONFIG.Motion.STEPS_PER_MM_Z
        )
    ...
    def set_deltaX(self, delta):
        self.deltaX = delta
        self.deltaX_usteps = round(delta * CONFIG.Motion.STEPS_PER_MM_XY)

    def set_deltaY(self, delta):
        self.deltaY = delta
        self.deltaY_usteps = round(delta * CONFIG.Motion.STEPS_PER_MM_XY)

    def set_deltaZ(self, delta_um):
        self.deltaZ = delta_um / 1000
        self.deltaZ_usteps = round((delta_um / 1000) * CONFIG.Motion.STEPS_PER_MM_Z)
    ...
    def _run_multipoint_single(self):
        ...
        for k in range(self.NZ):
            ...
            if self.configurationManager:
                ...
                saving_path = os.path.join(
                    current_path,
                    "camera1_" + file_ID + "." + CONFIG.Acquisition.IMAGE_FORMAT,
                )
                image_to_display = utils.crop_image(
                    image,
                    round(
                ...
                saving_path = os.path.join(
                    current_path,
                    "camera2_" + file_ID + "." + CONFIG.Acquisition.IMAGE_FORMAT,
                )
                image_to_display = utils.crop_image(
                    image,
                    round(
