...
import squid_control.control.microcontroller2 as microcontroller2
...
class OctopiGUI(QMainWindow):
    ...
    def __init__(self, is_simulation=False, *args, **kwargs):
        ...
        for i in range(len(channels)):
            ...
            self.camera[
                channels[i]
            ].set_software_triggered_acquisition()  # self.camera.set_continuous_acquisition()
        ...
        for i in range(len(channels)):
            ...
            self.configurationManager[channels[i]] = core.ConfigurationManager(
                filename=str(Path.home()) + "/configurations_" + channels[i] + ".xml"
            )
            ...
            self.imageSaver[channels[i]] = core.ImageSaver(
                image_format=CONFIG.Acquisition.IMAGE_FORMAT
            )
            # widgets
            self.cameraSettingWidget[channels[i]] = widgets.CameraSettingsWidget(
            ...
            self.cameraTabWidget.addTab(
                self.liveControlWidget[channels[i]], channels[i]
            )
        ...
        dock_display = {}
        for i in range(len(channels)):
            dock_display[channels[i]] = dock.Dock(
                "Image Display " + channels[i], autoOrientation=False
            )
