...
TOUPCAM_EVENT_STILLIMAGE = (
    0x0005  # snap (still) frame arrived, use PullStillImageXXXX to get this frame
)
...
TOUPCAM_OPTION_FRONTEND_DEQUE_LENGTH = (
    0x31  # frontend (raw) frame buffer deque length, range: [2, 1024], default: 4
)
...
TOUPCAM_OPTION_DUMP_CFG = (
    0x3F  # 0 = when camera is stopped, do not dump configuration automatically
)
...
#
TOUPCAM_OPTION_ANTI_SHUTTER_EFFECT = (
    0x4B  # anti shutter effect: 1 => disable, 0 => disable; default: 1
)
...
TOUPCAM_OPTION_AUTOEXPO_CONV = (
    0x50  # get auto exposure convergence status: 1(YES) or 0(NO), -1(NA)
)
TOUPCAM_OPTION_AUTOEXPO_TRIGGER = (
    0x51  # auto exposure on trigger mode: 0 => disable, 1 => enable; default: 0
)
...
TOUPCAM_IOCONTROLTYPE_GET_DEBOUNCERTIME = (
    0x0B  # debouncer time in microseconds, range: [0, 20000]
)
...
TOUPCAM_IOCONTROLTYPE_GET_TRIGGERDELAY = (
    0x0F  # Trigger delay time in microseconds, range: [0, 5000000]
)
...
TOUPCAM_IOCONTROLTYPE_GET_STROBEDURATION = (
    0x25  # Strobe duration time in microseconds, range: [0, 5000000]
)
...
TOUPCAM_IOCONTROLTYPE_GET_EXPO_ACTIVE_MODE = (
...
TOUPCAM_IOCONTROLTYPE_GET_EXEVT_ACTIVE_MODE = (
    0x35  # exposure event: 0 => specified line, 1 => common exposure time
)
TOUPCAM_IOCONTROLTYPE_SET_EXEVT_ACTIVE_MODE = 0x36
TOUPCAM_IOCONTROLTYPE_GET_OUTPUTCOUNTERVALUE = (
    0x37  # Output Counter Value, range: [0 ~ 65535]
)
...
TOUPCAM_AE_PERCENT_MIN = (
    0  # auto exposure percent; 0 or 100 => full roi average, means "disabled"
)
...
TOUPCAM_DYNAMIC_DEFECT_T1_MIN = (
    10  # dynamic defect pixel correction, threshold, means: 1.0
)
    ...
    class _ModelV2(ctypes.Structure):  # camera model v2 win32
        _fields_ = [
            ...
            (
                "maxfanspeed",
                ctypes.c_uint,
            ),  # maximum fan speed, fan speed range = [0, max], closed interval
...
class Toupcam:
    ...
    if sys.platform == "win32":
        ...
        __HISTOGRAM_CALLBACK = ctypes.WINFUNCTYPE(
            None, ctypes.c_void_p, ctypes.c_uint, ctypes.py_object
        )
    else:
        ...
        __HISTOGRAM_CALLBACK = ctypes.CFUNCTYPE(
            None, ctypes.c_void_p, ctypes.c_uint, ctypes.py_object
        )
    ...
    def PullImageV3(self, pImageData, bStill, bits, rowPitch, pInfo):
        if pInfo is None:
            self.__lib.Toupcam_PullImageV3(
                self.__h, pImageData, bStill, bits, rowPitch, None
            )
        else:
            x = self.__FrameInfoV3()
            self.__lib.Toupcam_PullImageV3(
                self.__h, pImageData, bStill, bits, rowPitch, ctypes.byref(x)
            )
            self.__convertFrameInfoV3(pInfo, x)

    def WaitImageV3(self, nWaitMS, pImageData, bStill, bits, rowPitch, pInfo):
        if pInfo is None:
            self.__lib.Toupcam_WaitImageV3(
                self.__h, nWaitMS, pImageData, bStill, bits, rowPitch, None
            )
        else:
            x = self.__FrameInfoV3()
            self.__lib.Toupcam_WaitImageV3(
                self.__h, nWaitMS, pImageData, bStill, bits, rowPitch, ctypes.byref(x)
            )
            self.__convertFrameInfoV3(pInfo, x)
    ...
    def PullStillImageV2(self, pImageData, bits, pInfo):
        if pInfo is None:
            ...
            x = self.__FrameInfoV2()
            self.__lib.Toupcam_PullStillImageV2(
                self.__h, pImageData, bits, ctypes.byref(x)
            )
            self.__convertFrameInfoV2(pInfo, x)
    ...
    def PullImageWithRowPitchV2(self, pImageData, bits, rowPitch, pInfo):
        if pInfo is None:
            ...
            x = self.__FrameInfoV2()
            self.__lib.Toupcam_PullImageWithRowPitchV2(
                self.__h, pImageData, bits, rowPitch, ctypes.byref(x)
            )
            self.__convertFrameInfoV2(pInfo, x)
    ...
    def TriggerSync(self, nTimeout, pImageData, bits, rowPitch, pInfo):
        ...
        if pInfo is None:
            ...
            x = self.__FrameInfoV3()
            self.__lib.TriggerSync(
                self.__h, nTimeout, pImageData, bits, rowPitch, ctypes.byref(x)
            )
            self.__convertFrameInfoV3(pInfo, x)
    ...
    def put_AutoExpoRange(self, maxTime, minTime, maxGain, minGain):
        return self.__lib.Toupcam_put_AutoExpoRange(
            self.__h,
            ctypes.c_uint(maxTime),
            ctypes.c_uint(minTime),
            ctypes.c_ushort(maxGain),
            ctypes.c_ushort(minGain),
        )
    ...
    def get_AutoExpoRange(self):
        ...
        self.__lib.Toupcam_get_AutoExpoRange(
            self.__h,
            ctypes.byref(maxTime),
            ctypes.byref(minTime),
            ctypes.byref(maxGain),
            ctypes.byref(minGain),
        )
    ...
    def put_MaxAutoExpoTimeAGain(self, maxTime, maxGain):
        return self.__lib.Toupcam_put_MaxAutoExpoTimeAGain(
            self.__h, ctypes.c_uint(maxTime), ctypes.c_ushort(maxGain)
        )
    ...
    def put_MinAutoExpoTimeAGain(self, minTime, minGain):
        self.__lib.Toupcam_put_MinAutoExpoTimeAGain(
            self.__h, ctypes.c_uint(minTime), ctypes.c_ushort(minGain)
        )
    ...
    def rwc_Flash(self, action, addr, pData):
        ...
        see democpp
        """
        self.__lib.Toupcam_rwc_Flash(
            self.__h, action, addr, ctypes.c_uint(len(pData)), pData
        )

    def write_Pipe(self, pipeId, pBuffer):
        self.__lib.Toupcam_write_Pipe(
            self.__h, pipeId, pBuffer, ctypes.c_uint(len(pBuffer))
        )
    ...
    def AAF(self, action, outVal):
        x = ctypes.c_int(0)
        self.__lib.Toupcam_AAF(
            self.__h, ctypes.c_int(action), ctypes.c_int(outVal), ctypes.byref(x)
        )
        return x.value
    ...
    def GetHistogram(self, fun, ctx):
        ...
        self.__cbhistogram = __class__.__HISTOGRAM_CALLBACK(
            __class__.__histogramCallbackFun
        )
        self.__lib.Toupcam_GetHistogramV2(
            self.__h, self.__cbhistogram, ctypes.py_object(self)
        )
    ...
    def __progressCallbackFun(percent, ctx):
        if __class__.__progress_fun:
            __class__.__progress_fun(percent, __class__.__progress_ctx)
    ...
    def Update(cls, camId, filePath, pFun, pCtx):
        ...
        if sys.platform == "win32":
            ...
            return cls.__lib.Toupcam_Update(
                camId.encode("ascii"), filePath.encode("ascii"), cls.__progress_cb, None
            )
    ...
    def __initlib(cls):
        if cls.__lib is None:
            try:  # Firstly try to load the library in the directory where this file is located
                ...
                if sys.platform == "win32":
                    cls.__lib = ctypes.windll.LoadLibrary(
                        os.path.join(dir_, "win", "x64", "toupcam.dll")
                    )
                elif sys.platform.startswith("linux"):
                    cls.__lib = ctypes.cdll.LoadLibrary(
                        os.path.join(dir_, "linux", "x64", "libtoupcam.so")
                    )
                else:
                    print(
                        "----------"
                        + os.path.join(dir_, "mac", "libtoupcam.dylib")
                        + "----------"
                    )
                    cls.__lib = ctypes.cdll.LoadLibrary(
                        os.path.join(dir_, "mac", "libtoupcam.dylib")
            ...
            if cls.__lib is None:
                if sys.platform == "win32":
                    cls.__lib = ctypes.windll.LoadLibrary(
                        "drivers and libraries/toupcam/windows/toupcam.dll"
                    )
                elif sys.platform.startswith("linux"):
            ...
            cls.__lib.Toupcam_PullImageV3.argtypes = [
                ctypes.c_void_p,
                ctypes.c_char_p,
                ctypes.c_int,
                ctypes.c_int,
                ctypes.c_int,
                ctypes.POINTER(cls.__FrameInfoV3),
            ]
            cls.__lib.Toupcam_WaitImageV3.restype = ctypes.c_int
            cls.__lib.Toupcam_WaitImageV3.errcheck = cls.__errcheck
            cls.__lib.Toupcam_WaitImageV3.argtypes = [
                ctypes.c_void_p,
                ctypes.c_uint,
                ctypes.c_char_p,
                ctypes.c_int,
            ...
            cls.__lib.Toupcam_SnapR.argtypes = [
                ctypes.c_void_p,
                ctypes.c_uint,
                ctypes.c_uint,
            ]
            ...
            cls.__lib.Toupcam_TriggerSync.argtypes = [
                ctypes.c_void_p,
                ctypes.c_uint,
                ctypes.c_char_p,
                ctypes.c_int,
            ...
            cls.__lib.Toupcam_put_AutoExpoRange.argtypes = [
                ctypes.c_void_p,
                ctypes.c_uint,
                ctypes.c_uint,
                ctypes.c_ushort,
            ...
            cls.__lib.Toupcam_get_AutoExpoRange.argtypes = [
                ctypes.c_void_p,
                ctypes.POINTER(ctypes.c_uint),
                ctypes.POINTER(ctypes.c_uint),
                ctypes.POINTER(ctypes.c_ushort),
            ...
            cls.__lib.Toupcam_rwc_Flash.argtypes = [
                ctypes.c_void_p,
                ctypes.c_uint,
                ctypes.c_uint,
                ctypes.c_uint,
            ...
            cls.__lib.Toupcam_AAF.argtypes = [
                ctypes.c_void_p,
                ctypes.c_int,
                ctypes.c_int,
                ctypes.POINTER(ctypes.c_int),
            ...
            cls.__lib.Toupcam_GetHistogramV2.argtypes = [
                ctypes.c_void_p,
                cls.__HISTOGRAM_CALLBACK,
                ctypes.py_object,
            ]
            cls.__lib.Toupcam_GigeEnable.restype = ctypes.c_int
            cls.__lib.Toupcam_GigeEnable.argtypes = [
            ...
            cls.__lib.Toupcam_Gain2TempTint.argtypes = [
                ctypes.c_int * 3,
                ctypes.POINTER(ctypes.c_int),
                ctypes.POINTER(ctypes.c_int),
            ]
            cls.__lib.Toupcam_TempTint2Gain.restype = None
            cls.__lib.Toupcam_TempTint2Gain.argtypes = [
                ctypes.c_int,
                ctypes.c_int,
                ctypes.c_int * 3,
            ]
