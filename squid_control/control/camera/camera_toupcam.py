...
import threading
import squid_control.control.toupcam as toupcam
from squid_control.control.toupcam_exceptions import hresult_checker
...
class Camera(object):
    ...
    def __init__(
        ...
        self.ROI_offset_x = CONFIG.CAMERA_CONFIG.ROI_OFFSET_X_DEFAULT
        self.ROI_offset_y = CONFIG.CAMERA_CONFIG.ROI_OFFSET_X_DEFAULT
        self.ROI_width = CONFIG.CAMERA_CONFIG.ROI_WIDTH_DEFAULT
        self.ROI_height = CONFIG.CAMERA_CONFIG.ROI_HEIGHT_DEFAULT
        ...
        self.thread_read_temperature = threading.Thread(
            target=self.check_temperature, daemon=True
        )
        ...
        self.OffsetX = CONFIG.CAMERA_CONFIG.ROI_OFFSET_X_DEFAULT
        self.OffsetY = CONFIG.CAMERA_CONFIG.ROI_OFFSET_X_DEFAULT
        self.Width = CONFIG.CAMERA_CONFIG.ROI_WIDTH_DEFAULT
        self.Height = CONFIG.CAMERA_CONFIG.ROI_HEIGHT_DEFAULT

        self.WidthMax = CONFIG.CAMERA_CONFIG.ROI_WIDTH_DEFAULT
        self.HeightMax = CONFIG.CAMERA_CONFIG.ROI_HEIGHT_DEFAULT
        ...
        if len(self.devices) > 0:
            ...
            if self.sn is not None:
                index = [
                    ...
                    if self.devices[idx].id == self.sn
                ][0]
            highest_res = (0, 0)
            self.res_list = []
            for r in self.devices[index].model.res:
            ...
            self.has_fan = (
                self.devices[index].model.flag & toupcam.TOUPCAM_FLAG_FAN
            ) > 0
            self.has_TEC = (
                self.devices[index].model.flag & toupcam.TOUPCAM_FLAG_TEC_ONOFF
            ) > 0
            self.has_low_noise_mode = (
                self.devices[index].model.flag & toupcam.TOUPCAM_FLAG_LOW_NOISE
            ) > 0
            ...
            )  # buffer created when setting resolution
            self._update_buffer_settings()

            if self.camera:
                if self.buf:
                    try:
                        self.camera.StartPullModeWithCallback(
    ...
    def set_exposure_time(self, exposure_time):
        ...
        # use_strobe = (self.trigger_mode == TriggerModeSetting.HARDWARE) # true if using hardware trigger
    ...
    def _update_buffer_settings(self):
        ...
        if (self.data_format == "RGB") & (self.pixel_size_byte != 4):
            bufsize = CONFIG._TDIBWIDTHBYTES(width * self.pixel_size_byte * 8) * height
        else:
    ...
    def send_trigger(self):
        if self._last_software_trigger_timestamp != None:
            if (time.time() - self._last_software_trigger_timestamp) > (
                1.5 * self.exposure_time / 1000 * 1.02 + 4
            ):
