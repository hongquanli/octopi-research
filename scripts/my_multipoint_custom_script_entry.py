...
def multipoint_custom_script_entry(
    multiPointWorker, time_point, current_path, coordinate_id, coordiante_name, i, j
):
    ...
    if (
        multiPointWorker.use_scan_coordinates
        and multiPointWorker.scan_coordinates_mm.shape[1] == 3
    ):
        ...
        if multiPointWorker.do_autofocus:
            ...
            if multiPointWorker.time_point == 0:
                ...
                configuration_name_AF = CONFIG.MULTIPOINT_AUTOFOCUS_CHANNEL
                config_AF = next(
                    (
                        config
                ...
                multiPointWorker.scan_coordinates_mm[coordinate_id, 2] = (
                    multiPointWorker.navigationController.z_pos_mm
                )
            ...
            # in subsequent scans, autofocus at the first FOV and offset the rest
                ...
                if coordinate_id == 0:
                    ...
                    multiPointWorker.autofocusController.autofocus()
                    multiPointWorker.autofocusController.wait_till_autofocus_has_completed()
                    multiPointWorker.scan_coordinates_mm[0, 2] = (
                        multiPointWorker.navigationController.z_pos_mm
                    )
                    offset = multiPointWorker.scan_coordinates_mm[0, 2] - z0
                    print("offset is " + str(offset))
                    multiPointWorker.scan_coordinates_mm[1:, 2] = (
                        multiPointWorker.scan_coordinates_mm[1:, 2] + offset
                    )
    ...
    # if z location is not included in the scan coordinates
        ...
        if multiPointWorker.do_reflection_af == False:
            # perform CONFIG.AF only if when not taking z stack or doing z stack from center
            if (
                (
            ...
            if multiPointWorker.reflection_af_initialized == False:
                ...
                # do contrast CONFIG.AF for the first FOV
                if multiPointWorker.do_autofocus and (
                    (multiPointWorker.NZ == 1)
                    or CONFIG.Z_STACKING_CONFIG == "FROM CENTER"
                ):
                    configuration_name_AF = CONFIG.MULTIPOINT_AUTOFOCUS_CHANNEL
    ...
    for k in range(multiPointWorker.NZ):
        ...
        file_ID = (
            ...
            + str(
                j
                if multiPointWorker.x_scan_direction == 1
                else multiPointWorker.NX - 1 - j
            )
            + "_"
            + str(k)
        )
        ...
        for config in multiPointWorker.selected_configurations:
            ...
            if "USB Spectrometer" not in config.name:
                ...
                if (
                    multiPointWorker.liveController.trigger_mode
                    ...
                    multiPointWorker.microcontroller.send_hardware_trigger(
                        ...
                        illumination_on_time_us=multiPointWorker.camera.exposure_time
                        * 1000,
                    )
                # read camera frame
                ...
                image = utils.crop_image(
                    image, multiPointWorker.crop_width, multiPointWorker.crop_height
                )
                ...
                image_to_display = utils.crop_image(
                    image,
                    round(
                        multiPointWorker.crop_width
                        * multiPointWorker.display_resolution_scaling
                    ),
                    round(
                        multiPointWorker.crop_height
                        * multiPointWorker.display_resolution_scaling
                    ),
                )
                multiPointWorker.image_to_display.emit(image_to_display)
                multiPointWorker.image_to_display_multi.emit(
                    image_to_display, config.illumination_source
                )
                if image.dtype == np.uint16:
                    saving_path = os.path.join(
                        current_path,
                        file_ID + "_" + str(config.name).replace(" ", "_") + ".tiff",
                    )
                    if multiPointWorker.camera.is_color:
                        if "BF LED matrix" in config.name:
                            if CONFIG.MULTIPOINT_BF_SAVING_OPTION == "RGB2GRAY":
                    ...
                    if multiPointWorker.camera.is_color:
                        if "BF LED matrix" in config.name:
                            if CONFIG.MULTIPOINT_BF_SAVING_OPTION == "Raw":
                                image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
                            elif CONFIG.MULTIPOINT_BF_SAVING_OPTION == "RGB2GRAY":
                                image = cv2.cvtColor(image, cv2.COLOR_RGB2GRAY)
                            elif (
                                CONFIG.MULTIPOINT_BF_SAVING_OPTION
                                == "Green Channel Only"
        ...
        multiPointWorker.coordinates_pd = pd.concat(
            [multiPointWorker.coordinates_pd, new_row], ignore_index=True
        )

        # register the current fov in the navigationViewer
        multiPointWorker.signal_register_current_fov.emit(
            multiPointWorker.navigationController.x_pos_mm,
            multiPointWorker.navigationController.y_pos_mm,
        )

        # check if the acquisition should be aborted
        if multiPointWorker.multiPointController.abort_acqusition_requested:
            multiPointWorker.liveController.turn_off_illumination()
            multiPointWorker.navigationController.move_x_usteps(
                -multiPointWorker.dx_usteps
            )
            multiPointWorker.wait_till_operation_is_completed()
            multiPointWorker.navigationController.move_y_usteps(
                -multiPointWorker.dy_usteps
            )
            multiPointWorker.wait_till_operation_is_completed()
            if multiPointWorker.navigationController.get_pid_control_flag(2) is False:
                _usteps_to_clear_backlash = max(
                    160, 20 * multiPointWorker.navigationController.z_microstepping
                )
                multiPointWorker.navigationController.move_z_usteps(
        ...
        if multiPointWorker.NZ > 1:
            ...
            if k < multiPointWorker.NZ - 1:
                multiPointWorker.navigationController.move_z_usteps(
                    multiPointWorker.deltaZ_usteps
                )
                multiPointWorker.wait_till_operation_is_completed()
                time.sleep(CONFIG.SCAN_STABILIZATION_TIME_MS_Z / 1000)
                multiPointWorker.dz_usteps = (
                    multiPointWorker.dz_usteps + multiPointWorker.deltaZ_usteps
                )
