...
class MultiPointWorker(QObject):
    ...
    def handle_z_offset(self, config, direction):
        if config.z_offset is not None:  # perform z offset for config, assume z_offset is in um
            if config.z_offset != 0.0:
                move_direction = 1 if direction else -1
                print("Moving to Z offset "+str(move_direction * config.z_offset))
                self.navigationController.move_z(move_direction * config.z_offset/1000)
                self.wait_till_operation_is_completed()
                time.sleep(SCAN_STABILIZATION_TIME_MS_Z/1000)
