...
class Application(tk.Frame):
    ...
    def toggle_scan(self,tog=[0]):
        tog[0] = not tog[0]
        if tog[0]:
            self.btn_scan.config(relief='sunken')
            for i in range( int(self.entry_Ny.get()) ):
                for j in range( int(self.entry_Nx.get()) ):
                    ...
                    self.btn_bf.config(relief='sunken')
                    ledON();
                    filename = ('_' + str(i) + '_' + str(j) )
                    camera.capture(filename + '_bf.jpeg')
                    ...
                    laserON();
                    filename = ('_' + str(i) + '_' + str(j) )
                    camera.capture(filename + '_fluorescent.jpeg')
                    ...
                    # move
                    self.x_move('f',int(self.entry_x_step.get()),float(self.entry_x_delay.get()))
                    self.label_xPos.config(text = str(j))
                    
                self.x_move('b',int(self.entry_Nx.get())*int(self.entry_x_step.get()),float(self.entry_x_delay.get()))
                self.label_xPos.config(text = '0')
                self.y_move('f',int(self.entry_y_step.get()),float(self.entry_y_delay.get()))
                self.label_yPos.config(text = str(i))
            
            self.y_move('b',int(self.entry_Ny.get())*int(self.entry_y_step.get()),float(self.entry_y_delay.get()))
            self.label_yPos.config(text = '0')
            self.btn_scan.config(relief='raised')
    ...
    def x_move(self,direction,steps,dt):
        ...
        self.label_xPos.config(text = str(self.x_pos/steps))
    ...
    def y_move(self,direction,steps,dt):
        ...
        self.label_yPos.config(text = str(self.y_pos/steps))
    ...
    def create_widgets(self):        
        ...
        self.label_xPos = tk.Label(self,text='0',width=6)
        ...
        self.label_yPos = tk.Label(self,text='0',width=6)
        ...
        self.btn_z_forward = tk.Button(self, text="Forward",
                              command=lambda:self.z_move('f',int(self.entry_z_step.get()),float(self.entry_z_delay.get())))
        self.btn_z_backward = tk.Button(self, text="Backward",
                              command=lambda:self.z_move('b',int(self.entry_z_step.get()),float(self.entry_z_delay.get())))
        self.label_zPos = tk.Label(self,text='0',width=6)
        ...
        self.btn_record.grid(row=17,column=0,columnspan=5,rowspan=2)
        
        # seperation
        self.label_seperator = tk.Label(self,text='  ')
        self.label_seperator.grid(row=19,column=0)
        
        # scan area def
        self.label_Nx = tk.Label(self,text='Nx')
        self.entry_Nx = tk.Entry(self,width = 3)
        self.label_Ny = tk.Label(self,text='Ny')
        self.entry_Ny = tk.Entry(self,width = 3)
        
        self.entry_Nx.insert(0,'4')
        self.entry_Ny.insert(0,'4')                                                      
        self.label_Nx.grid(row=20,column=0,sticky=tk.W)
        self.entry_Nx.grid(row=20,column=1,sticky=tk.W)
        self.label_Ny.grid(row=20,column=2,sticky=tk.W)
        self.entry_Ny.grid(row=20,column=3,sticky=tk.W)
        
        # Scan
        self.btn_scan = tk.Button(self, text="Scan", fg="black", bg = "yellow",
                                     width = 32, height = 2,
                                     command=self.toggle_scan)
        self.btn_scan.grid(row=21,column=0,columnspan=5,rowspan=2)
...
def autofocus(N,step,object):
  ...
  for i in range(N):
    j = j + 1
    # actuate
    object.z_move('f',step,0.001)
    sleep(0.005)
    img = np.empty((1088 * 1088 * 3,), dtype=np.uint8)
    ...
    img = img.reshape(1088,1088,3)
    ROI = img[540-256+1:540+256,540-256+1:540+256,1]
    lap = laplace(ROI)
    fm = mean(square(lap))
    print(fm)
    FM[i] = fm
  ...
  print(idx)
  # plt.plot(FM)
  object.z_move('b',step*j,dt)
  object.z_move('f',step*(idx+1),dt)
...
camera = PiCamera(resolution='1080x1080',sensor_mode=1,framerate = 30)
camera.iso = 400
camera.exposure_mode = 'off'
camera.shutter_speed = 50
camera.awb_mode = 'off'
camera.awb_gains = (2,1)
camera.start_preview(resolution=(1080, 1080),fullscreen=False, window=(0, 0, 1200, 1200))

# create GUI
root = tk.Tk()
root.geometry('400x500+1200+36')
